...
// Player object
const player = new Player(TILE_SIZE * 2, TILE_SIZE * 2, TILE_SIZE, TILE_SIZE);

// Game objects (enemies)
const enemies = ENEMY_POSITIONS.map(pos =>
    new Enemy(pos.x, pos.y, TILE_SIZE, TILE_SIZE)
);
...
function drawPath(path, baseTileSize) {
    ...
    if (!gameState.isMoving) {
        ...
        const currentPathIndex = path.findIndex(point =>
            point.x === currentTileX && point.y === currentTileY
        );
...
function drawEntities() {
    // Draw player
    player.draw(ctx, entityImages);

    // Draw enemies
    enemies.forEach(enemy => {
        if (!enemy.isActive) return; // Don't draw dead enemies
        enemy.draw(ctx, entityImages);
    });
}
